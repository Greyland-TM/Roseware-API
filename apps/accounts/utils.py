...
def update_or_create_ongoing_sync(
    type, action, should_sync_stripe, should_sync_pipedrive, sync_platform, owner
):
    CustomUser = apps.get_model("accounts", "CustomUser")
    OngoingSync = apps.get_model("accounts", "OngoingSync")

    owner = CustomUser.objects.get(pk=owner)
    # check for an ongoing roseware sync
    ongoing_sync = OngoingSync.objects.filter(
        type=type, action=action, owner=owner
    ).first()
    if ongoing_sync:
...
def create_customer_sync(customer, should_sync_stripe, should_sync_pipedrive):
    ...
    if should_sync_stripe:
        ...
        sync_stripe.apply(
            kwargs={"pk": customer.pk, "action": "create", "type": "customer"}
        )
