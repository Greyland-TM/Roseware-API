# ********************************
# This task is not going to be used by markit,
# But I'm leaving it so we can sell basic monday.com integration to other companies
# ********************************
...
@app.task
def save_to_monday(
    customer_pk, board_type, is_new, package_pk=None, customer_monday_id=None
):
    ...
    try:
        ...
        logger.info(f"Attempting to update monday.com board...")
        logger.info(
            f"Customer: {customer_full_name}, email: {customer.email}, pk: {customer.pk}, board_type: {board_type}"
        )
        ...
        monday_query_string = "query {{boards (ids: {}) {{owners {{id}} columns {{id title type}} }} }}".format(
            board_id
        )
        ...
        if is_new:
            monday_id = create_monday_entry(
                customer, board_id, url, headers, new_column_values
            )
        else:
            monday_id = update_monday_entry(
                customer, board_id, url, headers, customer.monday_id, new_column_values
            )

        # Check if contact was created / handle errors
        if not monday_id:
            logger.error("ERROR: Failed to create/update contact on monday.com")
            return False

        logger.info(
            f"Successfully synced {customer} to Monday.com {board_type} board..."
        )
        return monday_id
