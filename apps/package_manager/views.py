    ...
    def get(self, request):
        ...
        try:
            ...
            if not hasattr(user, "employee") or not user.employee:
                return Response(
                    {"ok": False, "error": "Not Authorized, Employees Only"},
            ...
            if not template:
                return Response(
                    {"ok": False, "error": "Service Package Template not found"},
            ...
            return Response(
                {
                    "ok": True,
                    "package_template": ServicePackageTemplateSerializer(template).data,
                },
        ...
        try:
            ...
            for field in required_fields:
                if field not in request.data:
                    return Response(
                        {"ok": False, "error": f"Missing required field: {field}"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            return Response(
                {
                    "ok": True,
                    "service-package-template": ServicePackageTemplateSerializer(
