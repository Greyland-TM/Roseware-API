...
class StripePaymentDetails(models.Model):
    ...
    def save(self, should_sync_stripe=True, *args, **kwargs):
        ...
        if should_sync_stripe:
            if is_new:
                print('Creating payment details in Stripe... (Check celery terminal)')
                ...
                print('Updating payment details in Stripe... (Check celery terminal)')
                sync_stripe.delay(kwargs={
                    'pk': self.pk,
                    'action': 'update',
                    'type': 'payment_details'
                })
    ...
    def delete(self, should_sync_stripe=True, *args, **kwargs):
        ...
        if should_sync_stripe:
            print('Deleting payment details in Stripe... (Check celery terminal)')
            sync_stripe.delay(kwargs={
                'pk': stripe_id,
                'action': 'delete',
                'type': 'payment_details'
...
class StripeSubscription(models.Model):
    customer = models.ForeignKey('accounts.Customer', on_delete=models.CASCADE)
    package_plan = models.ForeignKey('package_manager.PackagePlan', on_delete=models.CASCADE, blank=True, null=True)
    ...
    def save(self, should_sync_stripe=True, *args, **kwargs):
        ...
        if should_sync_stripe:
            if is_new:
                print('Creating subscription in Stripe... (Check celery terminal)')
                sync_stripe.apply(kwargs={
                    'pk': self.pk,
                    'action': "create",
                    'type': "subscription"
                ...
                print('Updating subscription in Stripe... (Check celery terminal)')
                sync_stripe.delay(kwargs={
                    'pk': self.pk,
                    'action': "update",
                    'type': "subscription"
